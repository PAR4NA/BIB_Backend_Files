...
UPDATEEMAIL='dougj007@verizon.net';
INFISTARVERSION='14-Sep-2016 15-39-50 - v0060';
...
if(!_return)exitWith
{
	diag_log format['<infiSTAR.de> %1 - serverCommandPassword NOT SET!   [dougj007@verizon.net - 14-Sep-2016 15-39-50 - v0060 - %2 - %3]',_t,serverName,productVersion];
...
{
	diag_log format['<infiSTAR.de> %1 - infiSTAR_AdminMenu.hpp is not in your MPMission or you forgot to #include it within your description.ext   [dougj007@verizon.net - 14-Sep-2016 15-39-50 - v0060 - %2 - %3]',_t,serverName,productVersion];
};
_test = [0,{}] execFSM 'call.fsm';
_test = [0,{}] execFSM 'call.fsm';
if(_test isEqualTo 0)exitWith
{
	_log = format['<infiSTAR.de> %1 - call.fsm missing in your MPmission!   [dougj007@verizon.net - 14-Sep-2016 15-39-50 - v0060 - %2 - %3]',_t,serverName,productVersion];
...
_blacklistedVariables append ['shit','_FNC_DECRYPT','whsh506_M41N'];
...
FNC_A3_RL = compileFinal "
	...
	_buildurl = format['http://htmlload.infistar.de/rl.php?owner=dougj007@verizon.net&owneruid=76561197960446555&infistarversion=14-Sep-2016 15-39-50 - v0060&servername=%1&logname=%2&logentry=%3',serverName,_logname,_logentry];
...
_fnc_RandomGen =
{
	...
	_fnc_actualGen = {
		...
		_main	= ['b','1','q','6','x','z','D','e','Z','g','z','m','H','k','D','y','6','k','2','V','M','2','S','P','R','x','J','Z','Q','l'];
...
['RANDOMVAR','----START-LINE----   (14-Sep-2016 15-39-50 - v0060 - dougj007@verizon.net)'] call FNC_A3_CUSTOMLOG;
...
_playerobj_check_loops = call _fnc_RandomGen;['RANDOMVAR',format['_playerobj_check_loops: %1',_playerobj_check_loops]] call FNC_A3_CUSTOMLOG;
...
_rcheck = call _fnc_RandomGen;['RANDOMVAR',format['_rcheck: %1',_rcheck]] call FNC_A3_CUSTOMLOG;
_ehidcheck = call _fnc_RandomGen;['RANDOMVAR',format['_ehidcheck: %1',_ehidcheck]] call FNC_A3_CUSTOMLOG;
_lastglitch = call _fnc_RandomGen;['RANDOMVAR',format['_lastglitch: %1',_lastglitch]] call FNC_A3_CUSTOMLOG;
_faaded = call _fnc_RandomGen;['RANDOMVAR',format['_faaded: %1',_faaded]] call FNC_A3_CUSTOMLOG;
			...
			if(alive _unit)then
			{
				...
				if(!_inTrader)then
				{
					...
					if(_damage >= 1)then
					{
						...
						['GODMODECHECK',_log] call FNC_A3_RL;
					...
					if(_damage > 0.2)then
					{
						...
						_lasthit = _unit getVariable ['lasthit',0];
						if(time - _lasthit > 1)then
						{
							_unit setVariable ['lasthit',time];
	...
	if(_isNormal)then
	{
		if(!isNull _origunit)then
		{
			if(alive _origunit)then
			{
				...
				{
					_ExileXM8IsOnline = _origunit getVariable 'ExileXM8IsOnline';
		...
		if(diag_tickTime > _displays_checked_timer)then
		{
			{
				_display = _x;
				_idd = ctrlIDD _display;
				{
					...
					if(call {
						...
						if(_buttonAction isEqualTo 'closeDialog 0')exitWith{false};
						if(_buttonAction isEqualTo 'closeDialog 0;')exitWith{false};
						...
						_log = format['MenuBasedHack BTNCODE :: %1 :: %2 :: %3',_idd,_idc,_buttonAction];
						_index = _displays_checked pushBackUnique _log;
						if(_index > -1)then
						{
			...
			_displays_checked_timer = diag_tickTime + 3;
		};
...
} forEach (nearestObjects [player,_checkthesedoorgates, 6]);
"; }; _A3AHstring = _A3AHstring + "
"; }; _A3AHstring = _A3AHstring + "
...

{
	...
	if(isNil'_var')then
	{
		_log = format['Variable [%1] is NIL! ',_x];
		[_name,_puid,'BAN',toArray(_log)] call _AHKickLog;
		[] call _AHKickOFF;
	}
	else
	{
		if!(_var isEqualType true)then
		{
			_log = format['Variable [%1] is WRONG TYPE [%2]! ', _x, typeName _var];
			[_name,_puid,'BAN',toArray(_log)] call _AHKickLog;
			[] call _AHKickOFF;
		};
	};
} forEach _bools_to_check;
...
_keyDownfncid = (finddisplay 46) displayAddEventHandler ['KeyDown',_keyDownfnc];

"; if(_useCustomFiredEventHandler)then{ _A3AHstring = _A3AHstring + "
...
"; if(_useCustomHandeDamageHandler)then{ _A3AHstring = _A3AHstring + "
...
_rcheck = player getVariable '"+_rcheck+"';
if(isNil'_rcheck')then{[_name,_puid,'rcheck',player] call _AHKickLog;};
};
_bigInputArray =
[
	[
		[_name,_uid,_AHKickOFF,_AHKickLog,_admins,_admin,_isNormal],
		{
			...
			"; if(!_KYLE_MODE)then{ _A3AHstring = _A3AHstring + "
			if(_isNormal)then {
				_badClasses = ['a3_m3editor','devcon','life_server','loki_lost_key','pooploop','rscrazzler','rsclistboxvg','vgdialog','sspcm'];
				_activatedAddons = [];
				...
				_cfgPatches = 'true' configClasses (configFile >> 'CfgPatches');
				{
					...
					if((toLower _configName) in _badClasses)then
					{
	...
	[
		[_name,_uid,_AHKickOFF,_AHKickLog,_admin,_isNormal,_49code,_blacklistedAdminVariables,"+str _blacklistedVariables+"],
		{
			...
			_origunit = objNull;
			_oldunit = objNull;
			_displays_checked_timer = 0;
			_displays_checked = [];

			_bools_to_check = ['ExileIsPlayingRussianRoulette','ExileClientIsHandcuffed','ExilePlayerInSafezone'];
			{
				private _var = missionNameSpace getVariable _x;
				if(isNil'_var')then
				{
			...
			_fnc_handleDamage = '
				...
				_ret = true;
				if(ExilePlayerInSafezone && {_unit call ExileClient_util_world_isInTraderZone})then{_ret = false;};
			...
			_fn_infiSTAR_block_von = {
				...
				{
					_switch = call {
						if(_keyCode in actionKeys 'VoiceOverNet')exitWith{true};
						if(_keyCode in actionKeys 'PushToTalk')exitWith{true};
						if(_keyCode in actionKeys 'PushToTalkAll')exitWith{true};
						if(_keyCode in actionKeys 'PushToTalkCommand')exitWith{true};
						if(_keyCode in actionKeys 'PushToTalkDirect')exitWith{true};
						if(_keyCode in actionKeys 'PushToTalkGroup')exitWith{true};
						if(_keyCode in actionKeys 'PushToTalkSide')exitWith{true};
						if(_keyCode in actionKeys 'PushToTalkVehicle')exitWith{true};
						if((uiNamespace getVariable ['ExileIsUsingVON', false])isEqualTo true)exitWith{true};
						false
					};
			...
			_keyUpfnc = {
				...
				if(_keyCode in (actionKeys 'TacticalView'))then{_handled = true;};
				if(_keyCode in (actionKeys 'User3'))exitWith{if(!isNil'fnc_chat_send')then{if(isNull (findDisplay -1340))then{createdialog 'infiSTAR_CHAT';};};};
				if(_keyCode isEqualTo 0x3D)exitWith{if(!isNil'fnc_workplace')then{[] call fnc_workplace;};};
				_handle
			};
			...
			_keyDownfnc = ""
				...
				_handled = false;
				call fn_infiSTAR_block_von;
				"; if(_block_glitch_actions)then{ _A3AHstring = _A3AHstring + "
				if(_keyCode in actionKeys 'GetOver' || _keyCode in actionKeys 'leanLeft'|| _keyCode in actionKeys 'leanLeftToggle' || _keyCode in actionKeys 'leanRight' || _keyCode in actionKeys 'leanRightToggle')then
				{
						...
						_fn_handle_glitched = {
							...
							[
								...
								[
									...
									'BLOCKED - Exile Basepart close!'
								]
							] call ExileClient_gui_toaster_addTemplateToast;
						};
						
						_exiledist = 3.5;
						if((cursorTarget isKindOf 'Exile_Construction_Abstract_Static' || cursorTarget isKindOf 'AbstractConstruction') && {cursorTarget distance player < _exiledist})exitWith{call _fn_handle_glitched;};
						_close = nearestObjects [player, ['Exile_Construction_Abstract_Static','AbstractConstruction'],_exiledist];
						if!(_close isEqualTo [])exitWith{call _fn_handle_glitched;};
					};
				};
				"; }; _A3AHstring = _A3AHstring + "
				if(_keyCode in (actionKeys 'TacticalView'))then{_handled = true;};
...
"; if(!_KYLE_MODE)then{ _A3AHstring = _A3AHstring + "
"; if((_wall_glitch_object)||(_wall_glitch_vehicle))then{ _A3AHstring = _A3AHstring + "
_glitchcode = {
	"; if(_wall_glitch_object)then{ _A3AHstring = _A3AHstring + "
	removeAllMissionEventHandlers 'EachFrame';
	addMissionEventHandler ['EachFrame',_fnc_wall_glitch_object];
	"; }; _A3AHstring = _A3AHstring + "
...
_iddchecks = {
	...
	if(!isNil '_uiNamespace_dynamicText')then
	{
		...
		{if(getPlayerUID _x != '')then{_whiteListed pushBack (_x call fnc_get_exileObjName);};} forEach playableUnits;
		{
			_ctrl = _x;
			_txt = ctrlText _ctrl;
...
_slowloop = {
	...
	_objects = nearestObjects [_pos, ['Exile_Construction_Abstract_Static','AbstractConstruction','Exile_Construction_Flag_Static'], 250];
	{
		if(isObjectHidden _x)then
		{
			...
			_log = format['HIDDEN OBJECT: %1, %2, %3, %4, %5',_x,typeOf _x,netId _x,position _x,mapGridPosition _x];
			[_name,_puid,'BAN',toArray(_log)] call _AHKickLog;
			[] call _AHKickOFF;
		};
	} forEach _objects;
...
_AHKickLog = _this select 3;
"; if(_wall_glitch_object)then{ _A3AHstring = _A3AHstring + "
if(isNil'"+_lastglitch+"')then{"+_lastglitch+"=time;};
if(isNil'"+_faaded+"')then{"+_faaded+"=false;};
...
scopeName 'MAIN';
{
	...
	{
		if(isNil'ExileCurrentBreachingObject')exitWith
		{
			_log = 'ExileCurrentBreachingObject is NIL';
			[_name,_puid,'BAN',toArray(_log)] call _AHKickLog;
			[] call _AHKickOFF;
		};
		if!(ExileCurrentBreachingObject isEqualType objNull)exitWith
		{
			_log = format['ExileCurrentBreachingObject is wrong type %1',typeOf ExileCurrentBreachingObject];
			[_name,_puid,'BAN',toArray(_log)] call _AHKickLog;
			[] call _AHKickOFF;
		};
		if(!isNull ExileCurrentBreachingObject)exitWith{};
		...
		if((_x isKindOf 'Exile_Construction_Abstract_Static') && !(_type in ['Exile_Construction_ConcreteStairs_Static','Exile_Construction_WoodStairs_Static']))exitWith
		{
			if(call fnc_check_if_enemy_base)then
			{
				if(isNil'"+_lastglitch+"')then
				{
					_log = 'lastglitch ("+_lastglitch+") is NIL';
					[_name,_puid,'BAN',toArray(_log)] call _AHKickLog;
					[] call _AHKickOFF;
				}
				else
				{
					if("+_lastglitch+" isEqualType 0)then
					{
						_timedif = time - "+_lastglitch+";
						if(_timedif > 5)then
						{
							_log = format['WALL GLITCH CHECK (intersects) - %1 @%2 %3',_type,mapGridPosition _x,position _x];
					...
					{
						_log = format['lastglitch ("+_lastglitch+") is wrong type: %1 (%2)',typeName "+_lastglitch+","+_lastglitch+"];
						[_name,_puid,'BAN',toArray(_log)] call _AHKickLog;
						[] call _AHKickOFF;
					};
				};
...
_head_back_left = player modelToWorldVisual [(_head select 0)-0.25, (_head select 1)-0.3, (_head select 2)+0.08];
_head_back_right = player modelToWorldVisual [(_head select 0)+0.15, (_head select 1)-0.3, (_head select 2)+0.08];
_head_front_left = player modelToWorldVisual [(_head select 0)-0.25, (_head select 1)+0.1, (_head select 2)+0.08];
_head_front_right = player modelToWorldVisual [(_head select 0)+0.15, (_head select 1)+0.1, (_head select 2)+0.08];
...

{
	...
	{
		if("+_faaded+")then{TitleText ['','PLAIN'];"+_faaded+"=false;};
	}
	else
	{
		{
...
if(_unit isEqualTo player)then
{
	...
	else
	{
		...
		[
			...
			[
				''infiSTAR ANTI-GLITCH:'',
				''Can not leave vehicle while it is rope attached!''
			]
		] call ExileClient_gui_toaster_addTemplateToast;
	};
};
				...
				if(time > LAST_NOTIFICATION_SHOWN)then
				{
					...
					
					{
						if(time > LAST_NOTIFICATION_SHOWN)then
						{
							if!(_x isEqualTo [])then
							{
								_x params[['_first',-1,[0]],['_replay',-1,[0]],['_delay',4,[0]],['_fsize',0.6,[0]],['_xpos',0,[0,'']],['_ypos',0,[0,'']],['_fcolor','#ff0000',['']],['_text','test',['']]];
								if(time > _first)then
								{
									if((time > _first)||(_first < 0))then
									{
										LAST_NOTIFICATION_SHOWN = time + _delay;
										if(!isStreamFriendlyUIEnabled)then
										{
											...
											_ctrl ctrlCommit 0;
											_ctrl ctrlSetStructuredText parseText format['<t size=''%1'' align=''left'' color=''%2''>%3</t>',_fsize,_fcolor,_text];
										};
										if(_replay < 0)then
										{
...
[
	[_admins],
	{
		...
		_devLog = format['infiSTAR.de AHAT %1 - 14-Sep-2016 15-39-50 - v0060 - server running: %2:3235 - AdminNameTag: "+str _AdminNameTag+"',VERSIONCHECKRESULT,call _GET_TIME_TIME];diag_log _devLog;
		"; if(!_HIDE_FROM_PLAYERS)then{ _A3AHstring = _A3AHstring + "
...
_fnc_l_on_c = {
	...
	if(_owner > 2)then
	{
		...
		if(!_isNormal)then
		{
			...
			infiSTAR_ADMINS_ALL = [];{if!(_x in _devs)then{infiSTAR_ADMINS_ALL pushBackUnique _x;};} forEach _admins;_owner publicVariableClient 'infiSTAR_ADMINS_ALL';
...
if!(_res isEqualTo '0')then{[_name,_uid,_owner,if(_res in ['','1'])then{'BLACKLISTED'}else{format['BLACKLISTED: %1',_res]}] call FNC_A3_infiSTARBAN;};
"";
...
_fnc_server_handle_mpmessage = {
	...
	if(_what isEqualTo 'rcheck')exitWith
	{
		...
		_player setVariable ['"+_rcheck+"',true,true];
	};
	...
	{
		['GLITCH_LOG',_log] call FNC_A3_RL;
		_log call fnc_add_survlog;
...
_fnc_AdminReqReal = {
	...
	if(typename _tokenreceived != 'STRING')exitWith{_log = format['AdminReqReal ERROR 1: %1',_this];['ADMINLOG',_log] call FNC_A3_RL;};
	if(_tokenreceived isEqualTo '')exitWith{_log = format['AdminReqReal ERROR 2: %1',_this];['ADMINLOG',_log] call FNC_A3_RL;};
	if(typename _clientUID != 'STRING')exitWith{_log = format['AdminReqReal ERROR 3: %1',_this];['ADMINLOG',_log] call FNC_A3_RL;};
	if(_clientUID in ['',' ','0'])exitWith{_log = format['AdminReqReal ERROR 4: %1',_this];['ADMINLOG',_log] call FNC_A3_RL;};
	if(typename _clientnetId != 'STRING')exitWith{_log = format['AdminReqReal ERROR 5: %1',_this];['ADMINLOG',_log] call FNC_A3_RL;};
	if(_clientnetId in ['','0:0'])exitWith{_log = format['AdminReqReal ERROR 6: %1',_this];['ADMINLOG',_log] call FNC_A3_RL;};
	if(typename _array != 'ARRAY')exitWith{_log = format['AdminReqReal ERROR 7: %1',_this];['ADMINLOG',_log] call FNC_A3_RL;};
	if(_array isEqualTo [])exitWith{_log = format['AdminReqReal ERROR 8: %1',_this];['ADMINLOG',_log] call FNC_A3_RL;};
	_admins = "+str _admins+";
	_exit = false;
...
"; }; _A3AHstring = _A3AHstring + "
	...
	if(_option isEqualTo 0)exitWith
	{
		...
		_vehicleObject spawn {uiSleep 5;if(!isNull _this)then{_this setDamage 0;_this allowDamage true;};};
	...
	"; if(_ryanzombies)then{ _A3AHstring = _A3AHstring + "
...
_fnc_server_handle_pre_mpmessage =
{
	private['_admins','_ObjFromnetId','_serverUID','_clientName','_clientID','_option'];
	params[['_tokenreceived',''],['_clientUID',''],['_clientnetId','0:0'],['_array',[]]];
	if(typename _tokenreceived != 'STRING')exitWith{_log = format['AH_KICKLOGSPAWN ERROR 1: %1',_this];['SURVEILLANCELOG',_log] call FNC_A3_RL;};
	if(_tokenreceived isEqualTo '')exitWith{_log = format['AH_KICKLOGSPAWN ERROR 2: %1',_this];['SURVEILLANCELOG',_log] call FNC_A3_RL;};
	if(typename _clientUID != 'STRING')exitWith{_log = format['AH_KICKLOGSPAWN ERROR 3: %1',_this];['SURVEILLANCELOG',_log] call FNC_A3_RL;};
	if(_clientUID in ['',' ','0'])exitWith{_log = format['AH_KICKLOGSPAWN ERROR 4: %1',_this];['SURVEILLANCELOG',_log] call FNC_A3_RL;};
	if(typename _clientnetId != 'STRING')exitWith{_log = format['AH_KICKLOGSPAWN ERROR 5: %1',_this];['SURVEILLANCELOG',_log] call FNC_A3_RL;};
	if(_clientnetId in ['','0:0'])exitWith{_log = format['AH_KICKLOGSPAWN ERROR 6: %1',_this];['SURVEILLANCELOG',_log] call FNC_A3_RL;};
	if(typename _array != 'ARRAY')exitWith{_log = format['AH_KICKLOGSPAWN ERROR 7: %1',_this];['SURVEILLANCELOG',_log] call FNC_A3_RL;};
	if(_array isEqualTo [])exitWith{_log = format['AH_KICKLOGSPAWN ERROR 8: %1',_this];['SURVEILLANCELOG',_log] call FNC_A3_RL;};
...
_from = 'dougj007@verizon.net';
_vers = '14-Sep-2016 15-39-50 - v0060';
...
_onPlayerConnected = {
	if(count _uid < 17)then
	{
		...
		['CONNECTLOG',_log] call FNC_A3_RL;
	}
	else
	{
		_steamName = call fnc_getSteamNameIfSaved;
...
"; }; _A3AHstring = _A3AHstring + "
	
	{
		if(!isNull _x)then
		{
			...
			_loggednetid = _x getVariable ['netId',''];
			if!(_loggednetid isEqualTo _netId)then
			{
				_x setVariable ['netId',_netId];
				_log = format['%1(%2) - %3, %4 - %5 - %6',_name,_uid,_x,_xtype,_netId,_owner];
			...
			if!(_admin)then
			{
				_MPHitID = _x getVariable ['MPHitID',-3];
				if(_MPHitID isEqualTo -3)then
				{
					_id = _x addMPEventHandler ['MPHit', { _this call fnc_mphit_handler }];
					_x setVariable ['MPHitID',_id];
				};
			};
			...
			_xRespect = _x getVariable 'ExileScore';
			if(!isNil'_xRespect')then
			{
				_xRespectCheck = _x getVariable 'ExileScoreCheck';
			...
			_playerMoney = _x getVariable 'ExileMoney';
			_lockerMoney = _x getVariable 'ExileLocker';
			if(!isNil'_playerMoney' && !isNil'_lockerMoney')then
			{
				_poptabsplayer = _playerMoney + _lockerMoney;
				_exilePopTabsCheck = _x getVariable 'ExilePopTabsCheck';
				if(isNil'_exilePopTabsCheck')exitWith
				{
					_x setVariable ['ExilePopTabsCheck', _poptabsplayer];
					_x setVariable ['ExileMoney', _playerMoney, true];
					_x setVariable ['ExileLocker', _lockerMoney, true];
				};
			...
			_isrunning = _x getVariable 'isrunning';
			...
			_playerobj_check_loops_1 = _x getVariable['"+_playerobj_check_loops+"_1',0];
			_playerobj_check_loops_1 = _playerobj_check_loops_1 + 1;
			_x setVariable['"+_playerobj_check_loops+"_1',_playerobj_check_loops_1];
			if(_xtype isEqualTo 'Exile_Unit_Player')then
			{
				if(_playerobj_check_loops_1 > 2)then
				{
					_rcheck = _x getVariable '"+_rcheck+"';
					if(!isNil'_rcheck')then
					{
						if(_playerobj_check_loops_1 > 8)then
						{
							_x setVariable['"+_rcheck+"',nil,true];
							_x setVariable['"+_playerobj_check_loops+"_1',0];
						};
					}
					else
					{
						_log = format['Player Clientside AntiHack not loaded or stopped! %1  @%2 %3',_xtype,mapGridPosition _x,getPos _x];
						[_name,_uid,'SLOG_SKICK',toArray(_log)] call "+_fnc_server_handle_mpmessage+";
					};
				};
				
				_playerobj_check_loops_2 = _x getVariable['"+_playerobj_check_loops+"_2',0];
				_playerobj_check_loops_2 = _playerobj_check_loops_2 + 1;
				_x setVariable['"+_playerobj_check_loops+"_2',_playerobj_check_loops_2];
				if(_playerobj_check_loops_2 > 5)then
				{
					_rating = rating _x;
					if(_rating < 500000)then
					{
						if(_rating isEqualTo 0)exitWith{};
						_log = format['Player Low Rating! %1 - %2 - @%3 %4',_rating,_xtype,mapGridPosition _x,getPos _x];
						[_name,_uid,'HLOG_SKICK',toArray(_log)] call "+_fnc_server_handle_mpmessage+";
					};
					_x setVariable['"+_playerobj_check_loops+"_2',0];
				};
			...
			{
				if(_playerobj_check_loops_1 > 4)then
				{
					_log = format['Player is %1 for too long - either not correctly loaded in or trying to run around as a ghost (invisible player object)   @%2 %3',_xtype,mapGridPosition _x,getPos _x];
					[_name,_uid,'SLOG_SKICK',toArray(_log)] call "+_fnc_server_handle_mpmessage+";
				};
				...
				{
					_log = format['WRONG PLAYER-TYPE: %1 should be Exile_Unit_Player   @%2 %3',_xtype,mapGridPosition _x,getPos _x];
					[_name,_uid,'HLOG_SKICK',toArray(_log)] call "+_fnc_server_handle_mpmessage+";
				};
			};
...
_log = format['[SERVER START] TIME: %1 || EMAIL: dougj007@verizon.net || VERSION: 14-Sep-2016 15-39-50 - v0060',time];
...
diag_log format['<infiSTAR.de RESTART_TIMER_SHUTDOWN> current mission time: %1',time];
diag_log format['<infiSTAR.de RESTART_TIMER_SHUTDOWN> current server time: %1',serverTime];
_timer4 = time + 10;
_fn_4 = {
	_runningtime = "+_USE_THIS_TIME_FUNCTION+";
...
"; if(_USE_RESTART_TIMER_SHUTDOWN)then{ _A3AHstring = _A3AHstring + "
		diag_log format['<infiSTAR.de> shutting down server.. after %1 minutes (RESTART_TIME_IN_M = %2 minutes in EXILE_AHAT_CONFIG.hpp)',_currentmessagetime,_restartTime];
...
_codePushBack = {
	...
	switch(_option)do
	{
		case 0: {
			...
			['VEHICLE_DUPE',_log] call FNC_A3_RL;
		...
		case 1: {
			_log = toString(_this select 2);
			_log = format['%1 (%2) - %3',_name,_uid,_log];
			['_____DUPE_____',_log] call FNC_A3_RL;
